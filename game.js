...
const PIPE_GAP = 210;
const PIPE_WIDTH = 70;
const PIPE_SPAWN_INTERVAL = 170; // frames between pipes
...
// --- Spawn Functions ---
function spawnPipe() {
    // Gap shrinks very slowly with difficulty (210 → min 140)
    const currentGap = Math.max(PIPE_GAP - (difficulty - 1) * 5, 140);
    const minY = 80;
    const maxY = GAME_HEIGHT - GROUND_HEIGHT - currentGap - 80;
...
function update() {
    ...
    // Player physics (gravity increases very slightly with difficulty)
    const currentGravity = GRAVITY + (difficulty - 1) * 0.015;
    ...
    // Spawn pipes (gap between spawns decreases slowly)
    const spawnRate = Math.max(PIPE_SPAWN_INTERVAL - (difficulty - 1) * 5, 90);
    if (frameCount % Math.round(spawnRate) === 0) {
        spawnPipe();
    }

    // Update pipes (speed increases gently)
    const currentSpeed = PIPE_SPEED + (difficulty - 1) * 0.12;
...
function checkPipeCollision(pipe) {
    ...
    const gap = pipe.gapSize || PIPE_GAP;
